...
export default function EventsBanner() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/events-banner`)
      .then((res) => res.json())
      .then((json) => {
        if (json.success) setContent(json.data);
      })
      .catch((err) => console.error("Failed to load events banner:", err))
  ...
  if (!content) {
    return (
      <section className="w-full py-20 text-center text-gray-400 bg-gray-50 border-b border-gray-100 text-sm">
        No Events banner configured yet — add it from the admin panel.
      </section>
    );
  }
        ...
        <div className="flex-1 min-w-0">
          ...
          <nav aria-label="Breadcrumb" className="mb-3">
            <p className="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-[#0f2418]/70 flex items-center flex-wrap gap-1">
              ...
              <span className="text-[#0a522c] font-bold">ALL EVENTS</span>
            </p>
          </nav>
