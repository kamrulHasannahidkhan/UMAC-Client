...
import { ArrowUpRight, GraduationCap, Briefcase, Calendar, Award } from "lucide-react";
...
export default function AlumniSection() {
  ...
  if (loading) {
    return (
      <section className="w-full py-20 bg-slate-50 text-slate-800">
  ...
  return (
    <section 
      className="w-full bg-slate-50 text-slate-900 py-16 sm:py-24 border-t border-slate-200 relative z-10"
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12 sm:mb-16 gap-4 border-b border-slate-200/80 pb-6">
          ...
          {content?.viewAllLink && (
            <a
              ...
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#0a5c30] hover:text-[#073d20] transition-colors group shrink-0"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
          {alumniList.map((person) => (
            ...
            >
              ...
              <div className="relative w-full h-64 sm:h-72 bg-slate-100 overflow-hidden">
                ...
                {/* Batch Badge */}
                {person.batch && (
                  <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-md px-3 py-1 rounded-full border border-slate-200 shadow-sm flex items-center gap-1 text-xs font-bold text-slate-800">
              ...
              <div className="p-5 flex-1 flex flex-col justify-between bg-white">
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-medium">
                  <span className="flex items-center gap-1">
                    <Award size={12} className="text-emerald-600" /> Verified UAMC Graduate
                  </span>
                </div>
              </div>
            </div>
          ...
          {alumniList.length === 0 && (
            <div className="col-span-full bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-xs">
              <GraduationCap size={36} className="mx-auto text-slate-400 mb-3" />
              <h3 className="font-semibold text-slate-800 text-base">No Alumni Listed</h3>
              <p className="text-sm text-slate-500 mt-1">
                Profiles will appear here once registered in the system.
              </p>
            </div>
          )}
        </div>
