...
export default function FacilityBanner({ activeTab }: { activeTab: string }) {
  const [content, setContent] = useState<FacilityBannerContent | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${API_URL}/api/facility-banner`)
      .then((res) => res.json())
      .then((json) => {
        if (json.success) setContent(json.data);
      })
      .catch((err) => console.error("Failed to load facility banner:", err))
  ...
  if (!content) {
    return (
      <section className="w-full py-20 text-center text-gray-400">
        No Facility banner yet — add it from the admin panel.
      </section>
    );
  }
  ...
  return (
    <section className="relative w-full h-[480px] flex items-center justify-center overflow-hidden bg-[#e4f3e6]">
      <Image src={content.bgImage} alt="Banner Background" fill className="object-cover object-center" sizes="100vw" priority />
      ...
      <div className="relative z-10 max-w-7xl w-[92%] mx-auto h-[280px] bg-[#9bcbb1]/70 backdrop-blur-md rounded-sm px-8 md:px-14 flex items-center justify-between shadow-sm">
        <div className="flex flex-col justify-center">
          <div className="text-sm md:text-base font-bold text-gray-800 tracking-wider mb-3 flex items-center gap-2 uppercase">
            ...
            <Link href="/facilities" className="hover:text-[#008d44] transition-colors">
              FACILITIES
            </Link>
            <span className="text-gray-700 font-semibold">&gt;&gt;</span>
            <span className="text-[#008d44] font-bold">{activeTab.toUpperCase()}</span>
          </div>

          <h1 className="text-5xl md:text-6xl font-serif text-[#2d3748]">
            {content.headingPlain} <span className="font-bold text-[#008d44]">{content.headingHighlight}</span>
          </h1>
        </div>
